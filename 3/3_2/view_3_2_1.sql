-- Agriculture-grain crops	321	Ar5
-- Hvert daa fulldyrka (artype 21) areal 100  (where high-quality cropland, 100)


DROP TYPE mess.function_3_2_1_type cascade;

CREATE TYPE mess.function_3_2_1_type AS 
(		  	
	gid bigint,
	value integer
);
...
--DROP FUNCTION mess.function_3_2_1(p_in geometry) cascade ;


CREATE OR REPLACE FUNCTION mess.function_3_2_1(p_in geometry) 
returns setof mess.function_3_2_1_type AS $BODY$
DECLARE
	returnrec mess.function_3_2_1_type;
	boundary geometry;
	num_rows int;
	default_value int = 0; -- unknown/unmapped areas
...
BEGIN
	...
	-- value is -100 where map exists but not cropland
...
drop table if exists mess.view_3_2_1; 
CREATE table mess.view_3_2_1 AS
  	SELECT (r1).function_3_2_1.gid::bigint, (r1).function_3_2_1.value::integer
	FROM (
		select mess.function_3_2_1(ST_GeomFromText('POLYGON EMPTY'))
	) as r1;

CREATE OR REPLACE view mess.view_3_2_1_quick AS
  	SELECT (r1).function_3_2_1.gid::bigint, (r1).function_3_2_1.value::integer
	FROM (
